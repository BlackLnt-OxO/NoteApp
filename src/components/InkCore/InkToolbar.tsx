/**
 * InkToolbar — brush-settings + tool panel content, shared by BOTH ink surfaces.
 *
 * This panel used to exist twice (`InfiniteInkCanvas/Toolbar.tsx` and
 * `PdfAnnotation/PdfToolbar.tsx`), ~392 of its lines byte-identical and the rest
 * differing only in how it reached the store. The body now lives here once; each
 * view supplies a thin adapter (see Toolbar.tsx / PdfToolbar.tsx) that maps its
 * own store's names onto these props:
 *
 *   canvas view: brush = 'marker'|'fountain'|…        brushSettings = {…}
 *   pdf view:    brushType = 'marker'|'fountain'|…    brush = {…}
 *
 * The two stores themselves are deliberately NOT merged — the PDF store carries
 * document/page/anchor state the canvas has no concept of, and vice versa.
 *
 * Rendered inside ToolbarShell, which owns docking/resize/collapse.
 */
...
import type { InkBrush, InkBrushType, InkEraserMode, InkInsertMode, InkSelectionMode, InkTool } from './inkTypes';
...
export interface InkToolbarProps {
  activeTool: InkTool;
  brushType: InkBrushType;
  brush: InkBrush;
  showDotGrid: boolean;
  selectionMode: InkSelectionMode;
  eraserMode: InkEraserMode;
  insertMode: InkInsertMode;
  canUndo: boolean;
  canRedo: boolean;

  /** The panel only ever selects select/pen/eraser/insert — the canvas's
   *  standalone 'text' tool is not reachable from here on either surface, which
   *  is what lets the PDF view (which has no text tool at all) share this. */
  setActiveTool: (t: Exclude<InkTool, 'text'>) => void;
  setBrushType: (b: InkBrushType) => void;
  setBrush: (p: Partial<InkBrush>) => void;
  setShowDotGrid: (v: boolean) => void;
  setSelectionMode: (m: InkSelectionMode) => void;
  setEraserMode: (m: InkEraserMode) => void;
  setInsertMode: (m: InkInsertMode) => void;
  undo: () => void;
  redo: () => void;

  /** Wording + action differ per surface (canvas clears the whole canvas, PDF
   *  clears only the current page), so the adapter supplies the confirm. */
  clearLabel: string;
  onClear: () => void;
}

const InkToolbar: React.FC<InkToolbarProps> = (p) => {
  const gfs = useNoteStore((s) => s.settings.fontSize);
  const {
    activeTool, brushType, brush, showDotGrid, selectionMode, eraserMode, insertMode,
    setActiveTool, setBrushType, setBrush, setShowDotGrid, setSelectionMode, setEraserMode, setInsertMode,
    undo, redo, canUndo, canRedo, clearLabel, onClear,
  } = p;

  const update = (patch: Partial<InkBrush>) => setBrush(patch);
  ...
  const onRootUp = (e: React.PointerEvent) => {
    ...
    if (atBtn && atBtn === pr.el) return; // same button → native click fires
    ...
    if (Math.hypot(e.clientX - pr.x, e.clientY - pr.y) <= 12) pr.el.click();
  };
  ...
  return (
    <div style={{ padding: '10px 14px 12px', color: 'var(--text-primary)', touchAction: 'none' }}
      ...
      <div style={{ display: 'flex', gap: '4px', ...sectionStyle }}>
        ...
        />
        {/* Both surfaces route text through insert-text mode (the canvas store
            also has a standalone 'text' tool, but its toolbar has always used
            'insert' here, so the shared behaviour is identical). */}
        <ExpandableToolButton
      ...
      {/* ---- Opacity ---- */}
      <div style={sectionStyle}>
        <div style={labelStyle(gfs)}><span>透明度</span><span>{Math.round(brush.opacity * 100)}%</span></div>
      ...
      {brushType === 'fountain' && (
        <div style={sectionStyle}>
          <div style={labelStyle(gfs)}><span>出墨速度</span><span>{Math.round(brush.inkSpeed * 100)}%</span></div>
          <input type="range" min={0} max={100} value={Math.round(brush.inkSpeed * 100)} onChange={(e) => update({ inkSpeed: Number(e.target.value) / 100 })} style={trackStyle} />
        </div>
      )}
