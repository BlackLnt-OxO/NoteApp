...
const NoteCard: React.FC<NoteCardProps> = ({ note, isDragGhost, isExpanded, onDragStart, onEditStart, onEditEnd }) => {
  ...
  const [cardW, setCardW] = useState(note.width || 130);
  const [cardH, setCardH] = useState(note.height || 100);
  ...
  useEffect(() => { setCardW(note.width || 130); setCardH(note.height || 100); }, [note.width, note.height]);
  useEffect(() => {
  ...
  const handleResizeDown = (e: React.PointerEvent) => {
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      const w = Math.max(90, resizeState.current.sw + dw);
      const h = Math.max(60, resizeState.current.sh + dh);
